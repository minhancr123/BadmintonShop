    ...
    function updateStatus(status) {
        ...
        if (confirm(`Bạn có chắc muốn cập nhật trạng thái đơn hàng thành "${statusNames[status]}"?`)) {
            ...
            .then(data => {
                if (data.success) {
                    location.reload();
                    showToast("Thành công", "Cập nhật đơn hàng thành công!", "success");
                } else {
                    showToast("Thất bại", data.message || "Có lỗi xảy ra khi cập nhật trạng thái", "error");
                }
            })
            .catch(error => {
                showToast("Thất bại", "Có lỗi xảy ra khi cập nhật trạng thái", "error");
            });
        }
    }
...
<?php echo $__env->make('admin.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\badminton-shop\resources\views/admin/orders/show.blade.php ENDPATH**/ ?>