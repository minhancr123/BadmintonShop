...
@section('title', 'Giỏ hàng - Badminton Shop')
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h5 class="mb-0">Sản phẩm trong giỏ ({{ count($cartItems) }} sản phẩm)</h5>
                            </div>
                            <div class="card-body p-0">
                                @foreach($cartItems as $item)
                                    <div class="cart-item border-bottom" data-id="{{ $item['id'] }}">
                                        <div class="row align-items-center p-3">
                                            <div class="col-md-2 text-center">
                                                <img src="{{ $item['product']->image ? asset('storage/' . $item['product']->image) : 'https://via.placeholder.com/100x100?text=No+Image' }}" 
                                                     class="img-fluid rounded" alt="{{ $item['product']->name }}" style="width: 80px; height: 80px; object-fit: cover;">
                                            </div>
                                            <div class="col-md-3">
                                                <h6 class="mb-1">{{ $item['product']->name }}</h6>
                                                <small class="text-muted">{{ $item['product']->category->name ?? 'N/A' }}</small><br>
                                                ...
                                                @elseif($item['product']->quantity < $item['quantity'])
                                                    <div class="mt-1">
                                                        <span class="badge bg-warning">Chỉ còn {{ $item['product']->quantity }} sản phẩm</span>
                                                    </div>
                                                @endif
                                            </div>
                                            <div class="col-md-2 text-center">
                                                <div class="price-info">
                                                    @if($item['product']->is_on_sale)
                                                        <span class="text-danger fw-bold">{{ number_format($item['product']->sale_price) }}₫</span><br>
                                                        <small class="text-decoration-line-through text-muted">{{ number_format($item['product']->price) }}₫</small>
                                                    @else
                                            ...
                                            <div class="col-md-2 text-center">
                                                <span class="fw-bold text-primary item-total">{{ number_format($item['subtotal']) }}₫</span>
                                            </div>
                            ...
                            <div class="card-footer">
                                <div class="row">
                                    <div class="col-md-6">
                                        <form action="{{ route('cart.clear') }}" method="POST" class="d-inline" onsubmit="return clearCartConfirm(event)">
                                            @csrf
                                    ...
                                    <div class="d-grid gap-2">
                                        @guest
                                            <div class="alert alert-info small">
                                                ...
                                                Vui lòng <a href="{{ route('login') }}" class="alert-link">đăng nhập</a> để tiến hành thanh toán.
                                            </div>
                                            <a href="{{ route('login') }}" class="btn btn-primary btn-lg">
                                                <i class="fas fa-sign-in-alt"></i> Đăng nhập để thanh toán
                                            </a>
                                        @else
                                            <a href="{{ route('checkout') }}" class="btn btn-success btn-lg" id="checkoutBtn">
